...
energy_seat_electric_train = 27
energy_seat_diesel_train = 70
energy_vehicle_electric_car = 184
energy_vehicle_electric_shuttle = 259
energy_vehicle_diesel_car = 686
energy_vehicle_diesel_shuttle = 911
#---------------x---------------------x-------------------x------------------------x----------------

#Carbon Index------------x--------------------------x----------------------x-----------------------x--

def getCarbonIndex(delta,b,l,dc,alpha,t_w,nu,E,vel_rat,power_law,capacity,dldcFuni,dgdcFbi,Funi,Fbi,ta,ts):
    if power_law:
        eta = getPowerLaw_etaVectorized(l,dc,nu,E,Funi,Fbi,dldcFuni)
    else:
        eta = b/delta
    CI_drt_uni = (1/eta)*(energy_vehicle_diesel_shuttle/energy_vehicle_diesel_car)*dldcFuni
    CI_drt_bi = (1/eta)*(energy_vehicle_diesel_shuttle/energy_vehicle_diesel_car)*2*beta*l*Fbi
    CI_pt = 4*(energy_seat_electric_train/energy_vehicle_diesel_car)*dgdcFbi/(np.pi*alpha)
...
def getConvenience(delta,b,l,dc,alpha,t_w,nu,E,vel_rat,power_law,capacity,dldcFuni,dgdcFbi,Funi,Fbi,ta,ts):
    V_tram = vel_rat
    Vm = V_tram
    mu = getmuVectorized(l,dc,nu,E,alpha,capacity,dgdcFbi)
